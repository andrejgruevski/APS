...
public class Heroes {
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards){
        SLLNode<Card> iterator = firstFriendCards.getFirst();
        SLLNode<Card> iterator2 = secondFriendCards.getFirst();

        SLLNode<Card> najjakaKarta = null; // jazol kade shto ke ja chuvame najjakata karta
        int max = 0; // pomoshna promenliva koja ke ni pomozhi pri sekoja iteracija da proveruvame dali go imame max dmg;

        while (iterator != null){
            if (iterator.element.damage() > max){
                max = iterator.element.damage();
                najjakaKarta = iterator;
            }
        ...
        secondFriendCards.insertBefore(firstFriendCards.delete(najjakaKarta),iterator2);
