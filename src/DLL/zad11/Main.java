...
public class Main {
    public static void function(DLL<Integer> lista, int m, int k){
        DLLNode<Integer> iterator = lista.getFirst();
        DLLNode<Integer> mElement = null;
        while (iterator != null) {
        ...
        for (int i =0; i<k; i++){
            if (mElement == lista.getLast()){
                lista.deleteLast();
                lista.insertFirst(m);
                mElement = lista.getFirst();
            }else{
                DLLNode<Integer> tmp = mElement.succ;
                lista.delete(mElement);
                lista.insertAfter(m,tmp);
                mElement = tmp.succ;
            }
        }
    }
